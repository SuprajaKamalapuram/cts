...
export class UserService {
  ...
  private followUrl='http://localhost:8072/follow'
  ...
  getFollowing(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:8072/follow/following/`+`${userid}`)
  }

  getFollower(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:8072/follow/follower/`+`${userid}`)
  }
